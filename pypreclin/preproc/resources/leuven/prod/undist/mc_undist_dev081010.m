...
%        fldata=flipdim(flipdim(M,1),3); 
    ...
    if mosaic==1
        ...
        % copy data
        modata=squeeze(M(:,:,:)*fftscale); 
        % save file
        eval(sprintf('datfile=''./bfloat/fu.bfloat'';'));
        fid = fopen(datfile,'wb');
        fwrite(fid,modata,'float32');
        status=fclose(fid);
        % write header
        eval(sprintf('hdrfile=''./bfloat/fu.hdr'';'));
        ...
        for slc=slin:slst:slfi
            % copy data
            sldata=squeeze(M(:,:,slc,:)*fftscale); 
            % save file
            eval(sprintf('datfile=''./bfloat/fu_%03.0f.bfloat'';',slc-1));
            fid = fopen(datfile,'wb');
            fwrite(fid,sldata,'float32');
            status=fclose(fid);
            % write header
            eval(sprintf('hdrfile=''./bfloat/fu_%03.0f.hdr'';',slc-1));
            fid = fopen(hdrfile,'wb');
            fprintf(fid,'%d %d %d %d # this line is for xds\n',Nx,Ny,Rep,1);
            fprintf(fid,'x %d \n',Nx);
            fprintf(fid,'y %d \n',Ny);
