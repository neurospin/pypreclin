...
%                V.im1(marl+1:end-maru,:)=im1(marl+1:end-maru,:);
                V.im1(marl+1:end-maru,:)=tim1(marl+1:end-maru,:).*tim1(marl+1:end-maru,:);

                % distorted image
                V.im2=zeros(size(tim2));
%                V.im2(marl+1:end-maru,:)=im2(marl+1:end-maru,:);
                V.im2(marl+1:end-maru,:)=tim2(marl+1:end-maru,:).*tim2(marl+1:end-maru,:);
...
%                imagesc(abs(im1'));
                ...
                subplot(py,px,pn);
                 xl=1:nc;
                plot(xl,stb00(xl),'.',xl,stb10(xl),'.',xl,stb20(xl),'.',xl,stb01(xl),'.')
                set(gca,'xLim',[0 ni+1]);
                set(gca,'yLim',[-1 1]);
                %
                pn=pn+1;
                ...
                line([marl marl],[0 96],'Color',[0 0 0],'Linewidth',2)
                line([rnx-maru rnx-maru],[0 96],'Color',[0 0 0],'Linewidth',2)
                %
                pn=pn+1;
                subplot(py,px,pn);
...
%        %
        ...
        % write b-header
        eval(sprintf('bhdrfile=''./bfloat/fu.hdr'';'));
