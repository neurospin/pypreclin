...
switch(spm_type(V_epi(1).dt(1)))
	case 'float64'
		dtype = 'double';
		mask_smooth = double(mask_smooth);
		Tx = zeros([xdim ydim zdim tdim], 'double');
		Tm = zeros([xdim ydim zdim], 'double');
 	case 'float32'
		dtype = 'single';
		mask_smooth = single(mask_smooth);
		Tx = zeros([xdim ydim zdim tdim], 'single');
		Tm = zeros([xdim ydim zdim], 'single');
	otherwise
		dtype = 'single';
		mask_smooth = single(mask_smooth);
		Tx = zeros([xdim ydim zdim tdim], 'single');
...
[tmp,vox] = max(Tm(:));
[xvox yvox zvox] = ind2sub(size(Tm), vox);
[tmp nt] = min(smooth(detrend(squeeze(Tx(xvox,yvox,zvox,:))).^2))

for n=1:tdim
	vol = Tx(:,:,:,n);
    background = sum(vol(:).*(1-mask_smooth(:)))/sum(1-mask_smooth(:));
    Tx(:,:,:,n) = background + (vol-background).*mask_smooth;
end;
...
%            mask2(:,1:20)=0;
    ...
    %                    tic;
% %                         W=mc_undist_loop_wouter_fast(V) ;
% % %                         W=mc_undist_loop_test6(V) ;
                        W=mc_undist_loop_test6(V) ;
    %                    toc;
