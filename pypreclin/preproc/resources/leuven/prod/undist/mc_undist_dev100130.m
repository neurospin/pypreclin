...
%            V.tmask=tmask;
            ...
            if doquad==4
                ...
                qim1(1:end/2+del,1:end/2,1)            =tim1(1:end/2+del,1:end/2);
                qim1(1:end/2+del,end/2+1-del:end,2)    =tim1(1:end/2+del,end/2+1-del:end);
                qim1(end/2+1-del:end,end/2+1-del:end,3)=tim1(end/2+1-del:end,end/2+1-del:end);
                qim1(end/2+1-del:end,1:end/2+del,4)    =tim1(end/2+1-del:end,1:end/2+del);
            end
            ...
            for qq=1:qnn
                ...
                if (length(find(tmask))>100);
                    ...
                    pow=1; % power used for optimization functions 
                    
                    % target image
                    tqim1=qim1(:,:,qq);
                    V.im1=zeros(size(tqim1));
                    V.im1(marl+1:end-marr,mart+1:end/2-marct)=tqim1(marl+1:end-marr,mart+1:end/2-marct).^pow;
                    V.im1(marl+1:end-marr,end/2+marcb+1:end-marb)=tqim1(marl+1:end-marr,end/2+marcb+1:end-marb).^pow;
                    
                    V.tmask=ones(size(im1));
                    V.tmask(V.im1==0)=0;
...
%                    V.im2(marl+1:end-marr,mart+1:end/2-marct)=tqim2(marl+1:end-marr,mart+1:end/2-marct).^2;
%                    V.im2(marl+1:end-marr,end/2+marcb+1:end-marb)=tqim2(marl+1:end-marr,end/2+marcb+1:end-marb).^2;
