% function [] = mc_undist_wouter2_20120524(fname_epi, fname_mask)
%
%  - fname_epi: file filter for epi files, like
% /data/local/DIR/Monkey/session/*_epi*.nii
% Loops over all files that match fname_epi
%
% - fname_mask: file filter for the mask, like
% /data/local/DIR/Monkey/session/*_msk.nii
% Takes the first file that matches fname_mask for all epi's
%

function [] = mc_undist_wouter2_20120524(fname_epi, fname_mask)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
home;
fprintf('Motion Compensation Undistortion: ''mc_undist_wouter2_files.m''\n');
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	addpath /data/fmri_monkey_03/PROJECT/Thomas/scripts/
...
dir_mask = fileparts(fname_mask);
dd=dir(fname_mask);
path_mask= fullfile(dir_mask, dd(1).name)

V_mask=spm_vol(path_mask)
mask = spm_read_vols(V_mask);

edir = fileparts(fname_epi);
dd2 = dir(fname_epi);

for nfile = 1:length(dd2)
	path_epi=[edir dd2(nfile).name];

	% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	fprintf('Motion Compensation Undistortion for ''%s''\n',path_epi);
	% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		for nc=1:tdim
			...
			for qq=1:qnn
				...
				if (((sum(im1(:))/sum(im2(:)) > 0.8) && (sum(im1(:))/sum(im2(:)) < 1.25)) && sum(im1(:)+im2(:) > 0) > 100)
					...
					% %                  % subroutine
					W=mc_undist_loop_wouter_20120524(V) ; % updated (28/06/2011)
