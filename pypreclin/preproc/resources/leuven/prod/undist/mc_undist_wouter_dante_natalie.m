function [] = mc_undist_wouter_dante_natalie(edir,runn)

%% Attention: script modified by natalie V1: 14.05.2012
% edir: give directory for files
% runn: give total number of runs, counts from 1st nii file in folder to
% last.

for xx11=runn % nat: added: loop trough runs
...
mask_smooth=smooth3(mask,'gauss',7);
mask_smooth=mask_smooth/max(mask_smooth(:));


% % pos=strfind(path_mask,'_');

dd2=dir([edir 'cr_*IMA*.nii']);

% path_epi=[edir dd2(1).name];
% modified by nat for looping trough files!
path_epi=[edir dd2(xx11).name];
...
if length(thres)
thres = prctile(sig,20);
end

if length(thres)
thres = prctile(sig,30);
end

if length(thres)
thres = prctile(sig,100);
end
...
options=2; % try to set to 2/3(with figure): brain divided into 4 parts, and UD options are aplied to them separately.
...
end % natalie added: ends the loop to go through runs.
