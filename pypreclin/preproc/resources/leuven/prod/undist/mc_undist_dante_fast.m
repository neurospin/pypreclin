function [] = mc_undist_dante_fast(edir)
...
Tx = zeros([xdim ydim zdim tdim], dtype);
...
% % 	T(:,:,:,n) = vol;
% %     Tx(:,:,:,n) = vol.*mask_smooth + background*(1-mask_smooth);
...
[tmp,vox]=max(Tm(:));
[voxx voxy voxz] = ind2sub(size(Tm), vox);

% % T5=smooth(detrend(squeeze(T(voxx, voxy, voxz,:))).^2);
T5=smooth(detrend(squeeze(Tx(voxx, voxy, voxz,:))).^2);
